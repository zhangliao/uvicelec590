...
int main(int argc, char* argv[])
{
    ...
    gtk_entry_set_text((GtkEntry*)(directory_entry), argv[1]);
    btn_back = gtk_button_new_with_label("<<");
	stpcpy(homepath,argv[1]);
	...
	gtk_window_set_title((GtkWindow*)window, "ELEC590 Online File System V0.01");
...
int init_file_list(GtkList* file_list, GtkEntry* directory_entry)
{
    ...
    while(enump = readdir(dir))
    {
        ...
        //        || (name_len == 2 && !strncmp(enump->d_name, "..", 2)) || (name_len == 4 && !strncmp(enump->d_name, ".git", 4)))
        if( enump->d_name[0] == '.')     
			continue;
...
void select_file_changed(GtkList* file_list, gpointer data)
{
    ...
    item_data_string = (gchar*)gtk_object_get_data(selected_item,list_item_data_key_string);
